...
const DEFAULT_WORDS = ["CLIENT GROWTH", "Welcome.", "This is where your business shines."]
...
export function ParticleTextEffect({ words = DEFAULT_WORDS, onComplete }: ParticleTextEffectProps) {
  ...
  const nextWord = useCallback((word: string, canvas: HTMLCanvasElement) => {
    ...
    const baseSize = Math.min(canvas.width * 0.07, 120)
    const fontSize = word.length > 25 ? baseSize * 0.5 : word.length > 15 ? baseSize * 0.65 : baseSize
    ...
    // Gold color for Client Growth brand
    const goldColors = [
      { r: 212, g: 168, b: 83 },  // #D4A853
      { r: 240, g: 234, b: 214 }, // #F0EAD6 cream
      { r: 196, g: 154, b: 42 },  // #C49A2A deep gold
    ]
    const newColor = goldColors[wordIndexRef.current % goldColors.length]
