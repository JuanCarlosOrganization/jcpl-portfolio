...
export const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    if ("outputColorSpace" in renderer) {
      (renderer as unknown as Record<string, unknown>).outputColorSpace = (THREE as unknown as Record<string, unknown>).SRGBColorSpace ?? (THREE as unknown as Record<string, unknown>).sRGBEncoding;
    }
    ...
    const updateMouse = (clientX: number, clientY: number) => {
      ...
      if (!rect) return;
      mouseTarget.set((clientX - rect.left) * currentDprRef.current, (rect.height - (clientY - rect.top)) * currentDprRef.current);
    };
    ...
    const adjustDprIfNeeded = (now: number) => {
      ...
      if (avgFps < 50) next = clamp(currentDprRef.current * 0.85, 0.6, base);
      else if (avgFps > 58 && currentDprRef.current < base) next = clamp(currentDprRef.current * 1.1, 0.6, base);
